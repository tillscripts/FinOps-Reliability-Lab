#!/usr/bin/env python3
"""
deploy_config_json.py
=====================
Network Configuration Management & Provisioning Tool — JSON Edition

Functionally identical to deploy_config_yaml.py.
The ONLY difference is the data-loading layer: JSON instead of YAML.

Workflow:
  1. Load structured device data from network_config.json
...
Dependencies:
    pip install jinja2 netmiko      (json is part of Python stdlib)

Usage:
    # Dry run (no SSH — safe for review)
    python deploy_config_json.py --dry-run

    # Live push to all devices
    python deploy_config_json.py --live

    # Live push to a single device
    python deploy_config_json.py --live --host SW-CORE-01
"""
...
DATA_FILE     = BASE_DIR / "network_config.json"
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. Load JSON data model
# ─────────────────────────────────────────────────────────────────────────────

def load_data(path: Path) -> dict:
    """
    Parse the JSON data model and return a Python dict.

    Key difference from YAML version:
        yaml.safe_load(fh)  →  json.load(fh)
    Both return a plain Python dict — the rest of the pipeline is identical.
    """
    log.info(f"Loading JSON data from: {path}")
    with open(path, "r") as fh:
        data = json.load(fh)

    # Strip the _comment field if present (JSON has no native comment syntax)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Network Config Provisioning Tool (JSON data source)"
    )
    parser.add_argument(
